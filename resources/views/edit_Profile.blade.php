...
<div class="grid grid-cols-11">
    <form action="{{ route('client.edit_profile', ['id' => auth()->guard('client')->user()->id]) }}" method="POST"
        enctype="multipart/form-data" class="p-8 w-full col-span-3">
        @csrf
        ...
        <div class="mb-4 col-span-3">
            <label for="imageUpload" class="cursor-pointer">
                <div class="relative w-40 h-40 rounded-full overflow-hidden bg-gray-900 hover:bg-opacity-50">
                    <img class="object-cover w-full h-full transition-opacity duration-200 ease-in-out hover:opacity-50"
                        src="{{ asset('storage/' . auth()->guard('client')->user()->image) }}" width="150" height="150"
                        alt="Profile Image" />
                    <div
            ...
            <input type="file" id="imageUpload" name="image" required style="display: none;" accept="image/*"
        ...
        @endif
        <button type="submit" class="btn bg-gray-500 p-2.5 text-white hover:bg-white hover:text-black">Change
            Profile</button>
    </form>
    
    <div class="col-span-3">
        <label for="name" class="block mb-2">Full Name</label>
        <input type="text" id="name" value="{{ auth()->guard('client')->user()->name }}" name="name" placeholder="Enter your full name"
        ...
        <input type="email" id="email" value="{{ auth()->guard('client')->user()->email }}" name="email" placeholder="Enter your email address"
            class="w-full px-3 py-2 mb-4 text-black placeholder-input text-input border border-border rounded-md focus:outline-none focus:ring ring-primary" />
        <button type="submit" class="w-full bg-gray-400 hover:bg-gray-500 py-2 rounded-md transition duration-300">Edit
            Profile</button>
